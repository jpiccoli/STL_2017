...
#define VTSEQ(ID) ("\x1b[" #ID "m")
	...
	const char* home_dir()
	{
      ...
      auto user_profile = _dupenv_s( &buf, &sz, "USERPROFILE" );
			//CHECK_F(user_profile != nullptr, "Missing USERPROFILE");
			//return user_profile;
      CHECK_F( buf != nullptr, "Missing USERPROFILE" );
      return buf;
#else // _WIN32
    ...
    // JAP - 03/17/2019
    FILE *file;
    auto err = fopen_s( &file, path, mode_str );
    if( err || file == nullptr ) {
	...
	Text ec_to_text(EcHandle ec_handle)
	{
		...
		with_newline[0] = '\n';
		strcpy_s(with_newline + 1, strlen( parent_ec.c_str() ), parent_ec.c_str());
